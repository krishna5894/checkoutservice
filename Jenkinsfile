    ...
    stage('Build and push image with Container Builder') {
      steps {
        container('gcloud-kubectl-docker') {
          sh "gcloud auth activate-service-account --key-file=extended-arcana-408715-b6ef439ab89b.json"
          sh "gcloud config set project extended-arcana-408715"
          sh "PYTHONUNBUFFERED=1 gcloud builds submit -t gcr.io/extended-arcana-408715/checkoutservice ."
        }
      }
   }
    stage('deployment'){
